...
def format_single_result(result: dict) -> str:
    """
    Formats the raw dictionary result from the AI analyzer into a 
    visually structured Markdown report with tables and progress bars.
    """
    if not result["success"]:
        return f"**Error:** {result['error']}"

    # Extracting data fields with default fallbacks
    d         = result["data"]
    ...
    # Mapping for visual indicators (Emojis and Status text)
    sentiment_map = {
        "Positive": "Positive",
        "Negative": "Negative",
        "Neutral":  "Neutral",
    ...
    # Calculating the progress bar visualization for sentiment score
    score_pct = int((score + 1) / 2 * 100)
    filled    = int(score_pct / 5)
    bar       = "▓" * filled + "░" * (20 - filled)
    
    # Formatting lists into Markdown strings
...
def run_single(text: str) -> str:
    """Validates input and triggers a single feedback analysis."""
...
def run_batch(text: str) -> str:
    """
    Processes multiple reviews, calculates aggregate statistics, 
    and generates a comprehensive batch report.
    """
    ...
    # Aggregating sentiment counts for the summary table
    ...
    # Building the Batch Summary Header
    out = f"""
...
---
"""
    # Appending individual breakdowns for each review in the batch
    for r in results:
...
# Configures design tokens for light and dark mode synchronization.
CSS = """
:root {
...
/* Sync with Gradio's native Dark Mode */
.dark {
...
with gr.Blocks(theme=gr.themes.Soft(primary_hue="indigo"), css=CSS, title="AI Feedback Analyzer") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.TabItem("Single Review"):
            ...
            gr.Markdown("<p style='font-size:12px;color:var(--c-text-faint);margin:12px 0 8px'>Try a sample:</p>")
            with gr.Row():
            ...
            # Event Handlers for Single Review
...
if __name__ == "__main__":
    # Dynamically fetch PORT for hosting platforms like Render
